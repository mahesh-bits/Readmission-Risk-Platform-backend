...
# Upgrade Plan: readmission-risk-platform backend (20260413134434)

- **Generated**: 2026-04-13 13:44:34
- **HEAD Branch**: N/A
- **HEAD Commit ID**: N/A
...
**JDKs**
- JDK 17.0.15: /Users/mmahes11/Library/Java/JavaVirtualMachines/ms-17.0.15/Contents/Home/bin (current project JDK, used by step 2)
- JDK 21: **<TO_BE_INSTALLED>** (required by steps 3 & 4)

**Build Tools**
- Maven: **<TO_BE_INSTALLED>** (no local Maven found; required for all compilation and test steps)
...
- Working branch: appmod/java-upgrade-20260413134434
- Run tests before and after the upgrade: true

## Upgrade Goals

- Upgrade Java from 17 to 21 (latest LTS)
...
| Spring Boot               | 3.2.4    | 3.2.4          | Spring Boot 3.2.x is fully compatible with Java 21      |
| maven-compiler-plugin     | 3.11.0   | 3.11.0         | Already compatible with Java 21; only source/target needs updating |
| jjwt-api/impl/jackson     | 0.11.5   | 0.11.5         | No Java 21 incompatibilities                            |
| spring-cloud-azure-starter-monitor | 6.1.0 | 6.1.0    | Compatible with Java 21                                 |
| PostgreSQL driver         | managed  | managed        | -                                                       |
| Flyway                    | managed  | managed        | -                                                       |

### Derived Upgrades

- Update `java.version` property in `pom.xml` from `17` to `21`
- Update `maven-compiler-plugin` source/target from `17` to `21`
- Note: Dockerfile already uses `eclipse-temurin:21` — no change required there

## Upgrade Steps

- **Step 1: Setup Environment**
  - **Rationale**: Install JDK 21 (target LTS) and Maven (no local Maven found) required for all build verification steps.
  - **Changes to Make**:
    - [ ] Install JDK 21
    - [ ] Install Maven 3.9+
  - **Verification**:
    - Command: `#appmod-list-jdks` to confirm JDK 21 available
    - Expected: JDK 21 present

---

- **Step 2: Setup Baseline**
  - **Rationale**: Establish pre-upgrade compile/test baseline with current JDK 17 for comparison.
  - **Changes to Make**:
    - [ ] Run baseline compile and tests with JDK 17
  - **Verification**:
    - Command: `JAVA_HOME=/Users/mmahes11/Library/Java/JavaVirtualMachines/ms-17.0.15/Contents/Home mvn clean test-compile -q && mvn clean test -q`
    - JDK: /Users/mmahes11/Library/Java/JavaVirtualMachines/ms-17.0.15/Contents/Home
    - Expected: Document compile/test results as baseline

---

- **Step 3: Upgrade Java from 17 to 21**
  - **Rationale**: Update `pom.xml` to target Java 21 bytecode. Spring Boot 3.2.4, maven-compiler-plugin 3.11.0, and all dependencies are already compatible with Java 21 — no other changes needed.
  - **Changes to Make**:
    - [ ] Update `java.version` property in `pom.xml` from `17` to `21`
    - [ ] Update `maven-compiler-plugin` `<source>` and `<target>` from `17` to `21`
  - **Verification**:
    - Command: `JAVA_HOME=<JDK21_PATH> mvn clean test-compile -q`
...
- **Step 4: Final Validation**
  - **Rationale**: Verify Java 21 upgrade goal is met, project compiles and all tests pass with JDK 21.
  - **Changes to Make**:
    - [ ] Clean rebuild with JDK 21
    - [ ] Run full test suite; fix any failures iteratively
  - **Verification**:
    - Command: `JAVA_HOME=<JDK21_PATH> mvn clean test -q`
    - JDK: JDK 21
    - Expected: Compilation SUCCESS + 100% tests pass

## Key Challenges

- **No local Maven installation**
  - **Challenge**: Maven is unavailable locally; needed for compilation and test verification.
  - **Strategy**: Install Maven via `#appmod-install-maven` in Step 1 before any build verification.

- **Low test coverage**
  - **Challenge**: No test classes found in `src/test/` — baseline test run will likely yield 0 tests.
  - **Strategy**: Treat 0/0 as a valid baseline; compilation success is the primary acceptance criterion.

## Plan Review

- All placeholders replaced.
- Direct Java 17→21 upgrade. No API removals between Java 17 and 21 affect this codebase. The project already uses `jakarta.*` namespaces (compatible with Spring Boot 3.x).
- Dockerfile already targets Java 21; this plan aligns the `pom.xml` to match.
